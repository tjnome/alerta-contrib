...
REPORTER_TAGS = app.config.get('REPORTER_TAGS', ["pamola_host", "pamola_externalid", "pamola_customerprefix"])
...
class GrayHandler(PluginBase):
    ...
    def pre_receive(self, alert, **kwargs):
        ...
        if (not tags.get('targethost')
            and tags.get('host') == tags['pamola_host']):
            LOG.debug(f"{HOSTNAME_TAGS} matches {REPORTER_TAGS} in alert: {alert}")

            if (tags.get('externalid') != tags['pamola_externalid']
                and tags.get('customerprefix') != tags['pamola_customerprefix']):
                LOG.debug(f"{CUSTOMER_TAGS} and {REPORTER_TAGS} does not match. Set tags for: {alert}")

                # Insert customerprefix, externalid.
                for key in CUSTOMER_TAGS: 
                    if key in tags: 
                        alert.tags.remove(key + '=' + tags[key])
                        alert.tags.append(key + '=' + tags['pamola' + "_" + key])

            # Remove reporter tags
            for key in REPORTER_TAGS: alert.tags.remove(key + '=' + tags[key])
            return alert

        filters = Filter.find_matching_filters(alert, "graylist")
        if not filters:
            # Impersonate (Amogus)
            LOG.warning(f'Filter does not match alert. Amogus SUS alert: {alert}')
            # Remove host, tagethost, externalid and customerprefix
            for key in CUSTOMER_TAGS: 
                if key in tags: alert.tags.remove(key + '=' + tags[key])
            for key in HOSTNAME_TAGS:
                if key in tags: alert.tags.remove(key + '=' + tags[key])

            for key in REPORTER_TAGS: 
                alert.tags.remove(key + '=' + tags[key])  # Remove reporter tags
                alert.tags.append(key.split('_', 1)[1] + '=' + tags[key]) # Insert host, customerprefix, externalid.
            return alert
        ...
        for f in filters:
            try:
                ...
                LOG.warning(f'filter has invalid attributes: {f.id} error: {e}')
                continue
    ...
    def receive_blackout(self, blackout: 'Blackout', **kwargs) -> 'Blackout':
        ...
        filters = Filter.find_matching_filters(parse(blackout), "graylist")
